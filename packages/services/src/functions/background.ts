      ...
      for (const frame of frames) {
        ...
        const newAvailableToSegment = unnest(
          segments
            ...
            .map((region) =>
              GridService.coordsForRegion([region.tl, region.br])
            )
        )
        ...
        const allPossibleTiles = uniq([
          ...newAvailableToSegment,
          ...missingTiles,
        ])
        ...
        admin.firestore().runTransaction(async (transaction) => {
          ...
          toRemoveSnapshots.forEach((snapshot) =>
        ...
        for (const region of regions) {
          const segment = await segmentService.provision(region, provisionHash)
          await qrService.createSegmentQr(frame.id, segment.id)
        }
